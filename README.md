...
| [`programs/`](programs/README.md) | the non-industrial half - everything home-made, with its sources, a folder each: the collector's own programs (`vvv/` - everything on his disks that no one else signed, the minesweeper inside), Домнич's calendar, Newton solver and ПИТОН, the BASIC games, VLAD & ALEX's Covox-style sampler, the МС 0515 in the МС0111 terminal complex, the KOI-7 recoder in C, the music-editor advert, the screen dumps, the BASIC programs of the other disks, the Pascal programs of no known author, and the schoolroom: AutoTeacher, Лицей №1 |
