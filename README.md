...
| `software/` | the industrial half: products that came without their sources - what one runs **on** the machine.  What runs the machine itself is in `kits/` |
...
| [`software/development/`](software/development/README.md) | the programming systems that came from outside the kits and are built here from their sources: DECUS C, in a build for the machine as it is and one for `EM.SYS`, with the tape's software tools |
| [`software/editors/`](software/editors/README.md) | the screen editor in its three cuts - `K13U`, `KED`, `R15` - with the manuals |
| [`software/diag/`](software/diag/README.md) | the factory exerciser, the instruction-timing meters, the scan-code viewer, the instruction emulator `GETEML` |
| [`software/print/`](software/print/README.md) | the printing programs |
| [`software/shells/`](software/shells/README.md) | the shells and file managers: `SCE`, EmeSoft's profShell `RS`, Rodionov's ROSA Commander |
| [`software/format/`](software/format/README.md) | the diskette formatters: DEC's `FORMAT`, Mihin's `FDZ`, the ОМЕГА and vvv104 builds of `FORML` and `FORMH` |
