...
| `PIC1.SAV` | PIC.SAV with eleven bytes changed - ten size constants 6->8 and one address 040->044: the same memory-mapping test rebuilt for a larger window/region. An engineer's parameter sweep preserved as two binaries.  Its source did not survive either | prompt; answers ?CSI-F-Файлненайден* |
