...
| `SYSLIB.OBJ` | DEC's system object library. Not interchangeable with the Pascal kit's `SYSLIB.OBJ` (`../pascal/`): a `PIP` linked against that one builds without a complaint and dies of an overlay error |
| `ODT.OBJ` | The octal debugger, to be linked into the program it debugs |

`MACRO` is not here: DEC's V5.4 source kit has no source for it. The kits' own are in `../macro/`.
