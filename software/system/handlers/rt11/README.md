...
DEC's `SL.SYS` is not here: it edits the line rightly but its screen control does not fit the machine's console. ОСА's and ОМЕГА's `SL.SYS` (`../osa/`, `../omega/`) load under these monitors.

`EM.SYS`, the EIS/FIS instruction emulator of the ОСА kit (`../osa/`), works under these monitors as it is: `SET EM SYSGEN`, then `SET EM ON`.

Not in the catalogue (`CATALOG.md`, `catalog.csv`) or in the cross-run table of `../README.md` yet.
