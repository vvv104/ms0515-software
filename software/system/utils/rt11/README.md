...
DEC's RT-11 V5.4 utilities, built from DEC's own sources by DEC's own command files, with DEC's LINK and libraries, inside the emulator (`rt11_devel/projects/rt11` of the emulator's repository). English messages, as DEC wrote them. `../dec/` is a different thing: DEC's originals as they were *found* on the Soviet kits' diskettes.
